...
module Dodopayments
  ...
  module Converter
    ...
    class << self
      ...
      def dump(target, value)
        target.is_a?(Dodopayments::Converter) ? target.dump(value) : Dodopayments::Unknown.dump(value)
      end
    end
  end
  ...
  module Union
    ...
    def coerce(value, state:)
      if (target = resolve_variant(value))
        return Dodopayments::Converter.coerce(target, value, state: state)
      end
      ...
      known_variants.each do |_, variant_fn|
        ...
        coerced = Dodopayments::Converter.coerce(target, value, state: state)
    ...
    def dump(value)
      if (target = resolve_variant(value))
        return Dodopayments::Converter.dump(target, value)
      end

      known_variants.each do
        target = _2.call
        return Dodopayments::Converter.dump(target, value) if target === value
      end
  ...
  class ArrayOf
    ...
    #
    # @return [Boolean]
    def ==(other) = other.is_a?(Dodopayments::ArrayOf) && other.nilable? == nilable? && other.item_type == item_type
      ...
      value
        .map do |item|
          ...
          in [true, nil]
            ...
            Dodopayments::Converter.coerce(target, item, state: state)
          end
        end
    end
    ...
    def dump(value)
      target = item_type
      value.is_a?(Array) ? value.map { Dodopayments::Converter.dump(target, _1) } : super
    end
  ...
  class HashOf
    ...
    #
    # @return [Boolean]
    def ==(other) = other.is_a?(Dodopayments::HashOf) && other.nilable? == nilable? && other.item_type == item_type
        ...
        .to_h do |key, val|
          ...
          v =
            ...
            in [true, nil]
              ...
              Dodopayments::Converter.coerce(target, val, state: state)
            end
    ...
    def dump(value)
      target = item_type
      value.is_a?(Hash) ? value.transform_values { Dodopayments::Converter.dump(target, _1) } : super
    end
  ...
  class BaseModel
    ...
    class << self
      ...
      private def add_field(name_sym, required:, type_info:, spec:)
        type_fn, info =
          case type_info
          in Proc | Dodopayments::Converter | Class
        ...
        nilable = info[:nil?]
        const = required && !nilable ? info.fetch(:const, Dodopayments::Util::OMIT) : Dodopayments::Util::OMIT
        ...
        define_method(name_sym) do
          target = type_fn.call
          value = @data.fetch(name_sym) { const == Dodopayments::Util::OMIT ? nil : const }
          state = {strictness: :strong, exactness: {yes: 0, no: 0, maybe: 0}, branched: 0}
          if (nilable || !required) && value.nil?
            nil
          else
            Dodopayments::Converter.coerce(
              target,
              value,
              state: state
            )
    ...
    class << self
      ...
      def coerce(value, state:)
        ...
        unless (val = Dodopayments::Util.coerce_hash(value)).is_a?(Hash)
          exactness[:no] += 1
          return value
        end
        exactness[:yes] += 1
        ...
        fields.each do |name, field|
          ...
          unless val.key?(api_name)
            if const != Dodopayments::Util::OMIT
              exactness[:yes] += 1
          ...
          converted =
            if item.nil? && (nilable || !required)
              ...
              coerced = Dodopayments::Converter.coerce(target, item, state: state)
              case target
              in Dodopayments::Converter | Symbol
                coerced
              else
                item
              end
      ...
      def dump(value)
        ...
        coerced.each do |key, val|
          ...
          in nil
            ...
            in :coerce
              ...
              target = type_fn.call
              acc.store(api_name, Dodopayments::Converter.dump(target, val))
            end
          end
        end
        ...
        known_fields.each_value do |field|
          ...
          next if mode == :coerce || acc.key?(api_name) || const == Dodopayments::Util::OMIT
