...
module Dodopayments
  ...
  module Converter
    ...
    class << self
      # @api private
      #
      # @param spec [Hash{Symbol=>Object}, Proc, Dodopayments::Converter, Class] .
      #
      ...
      def type_info(spec)
        ...
        in Proc
          spec
        in Dodopayments::Converter | Module | Symbol
          -> { spec }
        in true | false
      ...
      #
      # @param target [Dodopayments::Converter, Class]
      # @param value [Object]
      #
      # @return [Object]
      def coerce(target, value)
        case target
        in Dodopayments::Converter
          target.coerce(value)
        in Symbol
          case value
  ...
  class Unknown
    ...
    #
    # @return [Boolean]
    def self.==(other) = other.is_a?(Class) && other <= Dodopayments::Unknown
  ...
  module Enum
    ...
    # @return [Boolean]
    def ==(other)
      other.is_a?(Module) && other.singleton_class.ancestors.include?(Dodopayments::Enum) && other.values.to_set == values.to_set
    end
  ...
  module Union
    ...
    # @api private
    #
    # @param key [Symbol, Hash{Symbol=>Object}, Proc, Dodopayments::Converter, Class]
    #
    # @param spec [Hash{Symbol=>Object}, Proc, Dodopayments::Converter, Class] .
    #
    #   @option spec [NilClass, TrueClass, FalseClass, Integer, Float, Symbol] :const
    ...
    private def variant(key, spec = nil)
      variant_info =
        case key
        in Symbol
          [key, Dodopayments::Converter.type_info(spec)]
        in Proc | Dodopayments::Converter | Module | Hash
          [nil, Dodopayments::Converter.type_info(key)]
        end
    ...
    # @param value [Object]
    #
    # @return [Dodopayments::Converter, Class, nil]
    private def resolve_variant(value)
      ...
      in [Symbol, Hash]
        ...
        resolved.nil? ? Dodopayments::Unknown : resolved.call
      else
        nil
      end
    end
    ...
    # @return [Boolean]
    def ==(other)
      other.is_a?(Module) && other.singleton_class.ancestors.include?(Dodopayments::Union) && other.derefed_variants == derefed_variants
    end
    ...
    def coerce(value)
      if (variant = resolve_variant(value))
        return Dodopayments::Converter.coerce(variant, value)
      end
      ...
      known_variants.each do |_, variant_fn|
        ...
        case Dodopayments::Converter.try_strict_coerce(variant, value)
        in [true, coerced, _]
          return coerced
        in [false, true, score]
      ...
      variant.nil? ? value : Dodopayments::Converter.coerce(variant, value)
    end
    ...
    def dump(value)
      if (variant = resolve_variant(value))
        return Dodopayments::Converter.dump(variant, value)
      end
  ...
  class ArrayOf
    ...
    # @param type_info [Hash{Symbol=>Object}, Proc, Dodopayments::Converter, Class]
    #
    ...
    #
    # @return [Boolean]
    def ==(other) = other.is_a?(Dodopayments::ArrayOf) && other.item_type == item_type
    ...
    def coerce(value)
      ...
      in Enumerable unless value.is_a?(Hash)
        value.map { Dodopayments::Converter.coerce(type, _1) }
      else
        value
      end
    end
    ...
    def dump(value)
      ...
      case value
      in Enumerable unless value.is_a?(Hash)
        value.map { Dodopayments::Converter.dump(type, _1) }.to_a
      else
        value
      end
    end
    ...
    def try_strict_coerce(value)
      ...
      in Array
        ...
        mapped =
          value.map do |item|
            case Dodopayments::Converter.try_strict_coerce(type, item)
            in [true, coerced, score]
              tally += score
              coerced
            in [false, true, score]
    ...
    # @api private
    #
    # @return [Dodopayments::Converter, Class]
    ...
    def initialize(type_info, spec = {})
      @item_type_fn = Dodopayments::Converter.type_info(type_info || spec)
    end
  end
  ...
  class HashOf
    ...
    #
    # @return [Boolean]
    def ==(other) = other.is_a?(Dodopayments::HashOf) && other.item_type == item_type
    ...
    def coerce(value)
      ...
      in Hash
        value.to_h do |key, val|
          coerced = Dodopayments::Converter.coerce(type, val)
    ...
    def dump(value)
      ...
      in Hash
        value.transform_values do |val|
          Dodopayments::Converter.dump(type, val)
        end
      else
        value
      end
    ...
    def try_strict_coerce(value)
      ...
      in Hash
        ...
        mapped =
          value.transform_values do |val|
            case Dodopayments::Converter.try_strict_coerce(type, val)
            in [true, coerced, score]
              tally += score
              coerced
            in [false, true, score]
  ...
  class BaseModel
    ...
    class << self
      ...
      def reverse_map
        @reverse_map ||= (self < Dodopayments::BaseModel ? superclass.reverse_map.dup : {})
      end
      ...
      private def add_field(name_sym, required:, type_info:, spec:)
        type_fn, info =
          case type_info
          in Proc | Module | Dodopayments::Converter
            [Dodopayments::Converter.type_info({**spec, union: type_info}), spec]
          in Hash
            [Dodopayments::Converter.type_info(type_info), type_info]
          end
        ...
        define_method(name_sym) do
          ...
          Dodopayments::Converter.coerce(field_type, value)
        rescue StandardError
          name = self.class.name.split("::").last
          raise Dodopayments::ConversionError.new(
    ...
    class << self
      ...
      def coerce(value)
        case Dodopayments::Util.coerce_hash(value)
        in Hash => coerced
          new(coerced)
        else
          value
      ...
      def dump(value)
        unless (coerced = Dodopayments::Util.coerce_hash(value)).is_a?(Hash)
          return value
        end
        ...
        values = coerced.filter_map do |key, val|
          ...
          in nil
            ...
            in :coerce
              ...
              target = type_fn.call
              [api_name, Dodopayments::Converter.dump(target, val)]
            end
          end
        end.to_h
      ...
      def try_strict_coerce(value)
        case value
        in Hash | Dodopayments::BaseModel
          value = value.to_h
        else
        ...
        known_fields.each_value do |field|
          ...
          in [_, true]
            ...
            case Dodopayments::Converter.try_strict_coerce(target, item)
            in [true, coerced, score]
              tally += score
    ...
    def initialize(data = {})
      case Dodopayments::Util.coerce_hash(data)
      in Hash => coerced
        @data = coerced.to_h do |key, value|
          ...
          stored =
            case [type, value]
            in [Module, Hash] if type <= Dodopayments::BaseModel
              type.new(value)
            in [Dodopayments::ArrayOf, Array] | [Dodopayments::HashOf, Hash]
              type.coerce(value)
            else
              value
            end
